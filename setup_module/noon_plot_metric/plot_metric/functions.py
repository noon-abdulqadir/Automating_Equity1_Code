...
class BinaryClassification:
    ...
    def plot_precision_recall_curve(self, threshold=None, plot_threshold=True, beta=1, linewidth=2,
        ...
        lines.append(l)
        labels.append(f'PR curve (area = {round(pr_auc, 2)})')

        # Plot mean precision
        l, = plt.plot([0, 1], [mean(prec), mean(prec)], color=c_mean_prec,
                      lw=linewidth, linestyle=ls_mean_prec)
        lines.append(l)
        labels.append(f'Mean precision = {round(mean(prec), 2)}')
        ...
        if plot_threshold:
            plt.title(f'{title} (Threshold = {round(t, 2)})')
        else:
            plt.title(title)
    ...
    def plot_score_distribution(self, threshold=None, plot_hist_TN=True, kde_ksw_TN=None, label_TN='True Negative', c_TN_curve='green'):
        if kde_ksw_TN is None:
