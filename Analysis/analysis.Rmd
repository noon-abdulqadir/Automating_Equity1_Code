...
# bibliography: "/Users/nyxinsane/Library/CloudStorage/OneDrive-UvA/Automating Equity/Study 1/Study1_Code/Bias in Candidate Sourcing Typeset/Bias in Candidate Sourcing APA7/references.bib"
output:
    html_document:
    df_print: paged
editor_options:
...
## Import libraries and set , set, pandas

```{r Import libraries, message=TRUE, warning=FALSE, paged.print=TRUE}
# Load libraries
...
## Read data and order factors

```{r Read dataframes, message=TRUE, warning=FALSE, paged.print=TRUE}
...
## Aggregate dataframe on Job ID

```{r Make mean df, message=TRUE, warning=FALSE, paged.print=TRUE}
## Make mean df
...
## Set independent variables to factors

```{r Set factors, message=TRUE, warning=FALSE, paged.print=TRUE}
## Set factors
# DF
df$Gender <-
...
```{r Set variables, message=TRUE, warning=FALSE, paged.print=TRUE}
data <- df_mean
gender_iv = "Gender"
age_iv = "Age"
...
```{r Visualize, message=TRUE, warning=FALSE, paged.print=TRUE}
for (dv in dv_cols){
  strrep("=",80)
...
## Function to conduct ANOVA and OLS regression on mean-aggregated data

```{r Function to perform analysis, message=TRUE, warning=FALSE, paged.print=TRUE}
...
```{r ANOVA and OLS regression, message=TRUE, warning=FALSE, paged.print=TRUE}
for (iv in ivs){
  for (dv in dv_cols){
...
## Gender-Wamrth ANOVA and OLS regression

```{r Gender-Warmth ANOVA and OLS regression, message=TRUE, warning=FALSE, paged.print=TRUE}
#### Gender-Warmth
...
### Gender-Warmth Violinplot

```{r Gender-Warmth Violin plot, message=TRUE, warning=FALSE, paged.print=TRUE}
...
## Gender-Competence ANOVA and OLS regression

```{r Gender-Competence ANOVA and OLS regression, message=TRUE, warning=FALSE, paged.print=TRUE}
#### Gender-Competence
...
```{r Gender-Competence Violin plot, message=TRUE, warning=FALSE, paged.print=TRUE}
## Gender-Competence Violin plot
...
## Age-Warmth ANOVA and OLS regression

```{r Age-Warmth ANOVA and OLS regression, message=TRUE, warning=FALSE, paged.print=TRUE}
#### Age-Warmth
...
```{r Age-Warmth Violin plot, message=TRUE, warning=FALSE, paged.print=TRUE}
## Age-Warmth Violin plot
...
## Age-Competence ANOVA and OLS regression

```{r Age-Competence ANOVA and OLS regression, message=TRUE, warning=FALSE, paged.print=TRUE}
#### Age-Competence
...
```{r Age-Competence Violin plot, message=TRUE, warning=FALSE, paged.print=TRUE}
## Age-Competence Violin plot
...
## Multi-level Model - Hierarchical Regressions

```{r Multi-levl Log Reg, message=TRUE, warning=FALSE, paged.print=TRUE}
library(sjstats)
...
### Null model with random intercept at the job ad level

```{r Null model, message=TRUE, warning=FALSE, paged.print=TRUE}
...
x### First level model with category of sectors as demarcated by social
group segregation (i.e., Gender and Age) as level 1 predictor
(independent variable)

```{r First level model, message=TRUE, warning=FALSE, paged.print=TRUE}
lvl1_preds_model <- lmer(warmth ~ gender + (1 | job_id), data = df)
summary(lvl1_preds_model)
```

#### Check performance of first level model

```{r Check performance of first level model, message=TRUE, warning=FALSE, paged.print=TRUE}
performance::check_model(lvl1_preds_model)
```

#### Check first level model parameters

```{r Check first level model parameters, message=TRUE, warning=FALSE, paged.print=TRUE}
broom.mixed::tidy(lvl1_preds_model)
```

#### Plot first level model

```{r Plot first level model, message=TRUE, warning=FALSE, paged.print=TRUE}
...
#### Plot first level model coefficients

```{r Plot first level model coefficients, message=TRUE, warning=FALSE, paged.print=TRUE}
sjPlot::plot_model(lvl1_preds_model, show.p = T, show.values = T)
```

#### Comparing the null model and the first level model

```{r Comparing the null model and the first level model, message=TRUE, warning=FALSE, paged.print=TRUE}
htmltools::HTML(htmlreg(list(null_model, lvl1_preds_model)))
```

### Specifiy random slop based on category of sectors as demarcated by social group segregation (i.e., Gender and Age)

```{r Random slop model, message=TRUE, warning=FALSE, paged.print=TRUE}
rs_preds_model <- lmer(Warmth ~ Gender + (Gender | Job.ID), data = df)
summary(rs_preds_model)
```

#### Check performance of random slope model

```{r Check performance of random slope model, message=TRUE, warning=FALSE, paged.print=TRUE}
performance::check_model(rs_preds_model)
```

#### Check random slope model parameters

```{r Check random slope model parameters, message=TRUE, warning=FALSE, paged.print=TRUE}
broom.mixed::tidy(rs_preds_model)
```

#### Plot random slope model

```{r Plot random slope model, message=TRUE, warning=FALSE, paged.print=TRUE}
...
### Plot random slope model coefficients

```{r Plot random slope model coefficients, message=TRUE, warning=FALSE, paged.print=TRUE}
sjPlot::plot_model(rs_preds_model, show.p = T, show.values = T)
```

### Comparing the null model, the first level model, and the random slope model

```{r Comparing the null model, the first level model, and the random slope model, message=TRUE, warning=FALSE, paged.print=TRUE}
htmltools::HTML(htmlreg(list(null_model, lvl1_preds_model, rs_preds_model)))
```

# ANOVA to compare models

```{r ANOVA to compare models, message=TRUE, warning=FALSE, paged.print=TRUE}
...
\`\`\`{r} myvars \<- c("Warmth", "Warmth_Probability", "Competence",
"Competence_Probability") new_df \<- df[myvars]

z_scores \<- as.data.frame(sapply(new_df, function(new_df)
(abs(new_df-mean(new_df))/sd(new_df))))

inside(M, df \<- new_df)

inside(M, { df \<- df %\>% mutate(new_new = branch(z_score_cal, "1.96"
\~ z_scores[!rowSums(z_scores\>1.96),], "2.58" \~
z_scores[!rowSums(z_scores\>2.58),], "3.29" \~
z_scores[!rowSums(z_scores\>3.29),])
...
```{r Save as .r file, message=TRUE, warning=FALSE, paged.print=TRUE}
